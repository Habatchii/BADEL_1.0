...
"""A Code Intelligence Language Engine for the BADEL language.
...
Module Usage:
    from cile_badel import scan
    mtime = os.stat("bar.badel")[stat.ST_MTIME]
    content = open("bar.badel", "r").read()
    scan(content, "bar.badel", mtime=mtime)
"""
...
class BADELCILEError(CILEError):
    pass
...
log = logging.getLogger("cile.badel")
...
def scan_buf(buf, mtime=None, lang="BADEL"):
    """Scan the given BADELBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {BADELBuffer} is the BADEL buffer to scan
    ...
    """
    # Dev Notes:
    # - This stub implementation of the BADEL CILE return an "empty"
    #   summary for the given content, i.e. CIX content that says "there
    #   are no code elements in this BADEL content".
    # - Use the following command (in the extension source dir) to
    #   debug/test your scanner:
    #       codeintel scan -p -l BADEL <example-BADEL-file>
    ...
    # Dev Note:
    # This is where you process the BADEL content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-BADEL-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for BADEL content, you may
    #   want to use it.
