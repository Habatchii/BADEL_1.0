...
lang = "BADEL"
log = logging.getLogger("codeintel.badel")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "BADEL-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
        "transcribe,",
...
# lexers. You must define a Lexer class for lang BADEL. If Komodo's
# scintilla lexer for BADEL is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class BADELLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for BADEL is one of Komodo's existing C++ lexers
...
#   class BADELLexer(Lexer):
#       lang = lang
#       def __init__(self):
#           self._properties = SilverCity.PropertySet()
#           self._lexer = SilverCity.find_lexer_module_by_id(ScintillaConstants.SCLEX_BADEL)
#           self._keyword_lists = [
...
# examples of usage.
class BADELLangIntel(LangIntel):
    lang = lang
...
# BADEL content and returns a description of the code in that file.
# See "cile_badel.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If BADEL is
...
class BADELCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_badel
        return cile_badel.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=BADELLexer(),
        buf_class=BADELBuffer,
        langintel_class=BADELLangIntel,
        import_handler_class=None,
        cile_driver_class=BADELCILEDriver,
