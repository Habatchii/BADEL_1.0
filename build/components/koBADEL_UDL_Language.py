# Registers the BADEL language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koBADELLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language BADEL")
    registry.registerLanguage(KoBADELLanguage())

class KoBADELLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "BADEL"
    lexresLangName = "BADEL"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "a289339f-f8ed-4726-986b-dc9e9ec4bfc2"
    defaultExtension = 'badel'
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'BADEL'}
